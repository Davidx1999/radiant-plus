export const EndView = (message, pattern, abandonou) => {
    const accentColor = "text-radiant-blue";
    const accentBg = "bg-radiant-blue";
    const btnColor = "bg-[#23293b]";
        ...
        <div class="min-h-screen flex flex-col items-center justify-start p-6 animate-fade-in bg-brand-bg text-center overflow-y-auto">
            ...
            'Concordo Totalmente'
                                ...
                                <label class="flex items-center gap-4 cursor-pointer group p-3 bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-colors">
                                    <input type="radio" name="p_cansativo" value="${label}" onchange="window.atualizarPesquisaFinal('cansativo', this.value)" class="w-6 h-6 accent-radiant-blue cursor-pointer">
            ...
            'Concordo Totalmente'
                                ...
                                <label class="flex items-center gap-4 cursor-pointer group p-3 bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-colors">
                                    <input type="radio" name="p_seguro" value="${label}" onchange="window.atualizarPesquisaFinal('seguro', this.value)" class="w-6 h-6 accent-radiant-blue cursor-pointer">
                                    <span class="text-sm text-white/70 group-hover:text-white">${label}</span>
                                </label>
                            `).join('')}
                        </div>
                    ...
                    <div class="mb-10 border-t border-white/5 pt-8">
                        ...
                        <div class="flex flex-col gap-4">
                            ${['Sim', 'Não', 'Talvez'].map(v => `
                                <label class="flex items-center gap-4 cursor-pointer group p-3 bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-colors">
                                    <input type="radio" name="p_voltaria" value="${v}" onchange="window.atualizarPesquisaFinal('voltaria', this.value)" class="w-5 h-5 accent-radiant-blue cursor-pointer">
                    ...
                    <div class="mt-12 pt-4 border-t border-white/5">
                        ...
                        <button id="btn-finish-telemetry" onclick="validarEEnviar(${abandonou})" class="w-full py-5 ${accentBg} hover:opacity-90 text-white font-black rounded-xl transition-all shadow-2xl uppercase tracking-widest text-lg">
